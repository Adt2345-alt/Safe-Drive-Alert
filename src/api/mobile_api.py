...
def report_defect():
    ...
    Inserts a new obstacle dynamically into the simulated road environment!
    """
    if sys_manager is None:
        return jsonify({"error": "System not initialized"}), 500
    ...
    # Find closest node on the simulator path and calculate distance
    sim = sys_manager.road_sim
    closest_distance = 0.0
    min_dist = float('inf')
    
    # Simple search for closest segment distance along path
    for i in range(len(sim.route) - 1):
        coord = sim.route[i]
        d = sim._distance_between_coords((lat, lon), coord)
        if d < min_dist:
            min_dist = d
            # Rough approximation of distance along path
            # We can calculate distance along path by summing segment distances
            accumulated = 0.0
            for j in range(i):
    ...
    logger.info(f"Mobile Report: Registered new {defect_type.upper()} [ID: {new_id}] at ({lat:.6f}, {lon:.6f}) via mobile API")
